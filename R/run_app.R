...
#' \pkg{writexl}) are declared in `Suggests` rather than `Imports`, so the core
#' data-processing API can be installed without them. `run_app()` checks they
#' are present and stops with an actionable message if any are missing.
#'
